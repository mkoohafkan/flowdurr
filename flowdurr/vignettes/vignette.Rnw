...
ln3params
@
The function also accepts named arguments for passing to lmom::pelpe3 or lmom::pelln3. The following example shows how to fit a 3-parameter lognormal distribution with a lower bound of 0: 
<<results=verbatim,echo=TRUE>>=
# note that a 0-bound distribution can also be fitted
ln0params = get_parameters(peakflows, distr='ln3', bound=0)
ln0params
@
