refine_parameters <-
structure(function#Refine parameter estimates using MLE
...
### estimate will be refined with other parameters.
  ...
  mygenoud = function(fn, par, ...){
    ...
    res = rgenoud::genoud(fn, starting.values=par, Domains=get_domain(par), ...)
  ...
  for(n in names(flowdata)){
    d = flowdata[[n]][!is.na(flowdata[[n]])]
	p = paramfunc(paramdata[[n]])
	p['pop.size'] = 2000
	p['hard.generation.limit'] = FALSE
	p['gradient.check'] = TRUE
    p['boundary.enforcement'] = 0	
    fits[[n]] = try(fitdistrplus::fitdist(d, distr, start=p$start, 
	                  fix.arg=p$fix.arg, custom.optim=mygenoud, 
					  nvars=length(p$start), pop.size=p$pop.size, 
